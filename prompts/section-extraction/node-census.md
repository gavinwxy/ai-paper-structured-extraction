...
single **role** drawn from five search clusters; the node's type is derived from that role
...
## The guiding principle: trace the method's life through five clusters

Every referenceable node plays one argumentative role, and the roles group into five clusters. Sweep them in order — this is how you find everything without flooding the list:
...
5. **the_problem** (what it fixes) — the unresolved question or unmet need the work addresses.
...
- `role` — one of the thirteen roles above.
...
- `cite_keys` — the in-text bibliography citation marker(s) attached to this node, as **bare keys** (format and role rules in the schema). Take the marker where the node is introduced or tabulated (a results-table row "GNMT + RL [31]" → `["31"]`); multiple keys are allowed when several citations introduce the node. Use `[]` for any root (`contribution`/`contribution_resource`/`contribution_finding`) and every `component` (your own work), for `task`, `metric`, and `problem` nodes, and whenever no citation is attached. This is what later links the node to its bibliography entry, so take the marker verbatim.
...
- Per-dataset/per-subtask rows of a single benchmark suite, and near-duplicate size/config variants of one baseline family (census the family once, under its strongest variant). This never licenses dropping a distinct baseline system in a results table — every distinct compared system is `compared_against`.

**Root:** the paper's primary deliverable is a root node — `contribution`, `contribution_resource`, or `contribution_finding` (cluster 1 defines each). Almost always there is **exactly one**: if several methods could qualify, only the overall primary one is the root; every part is a `component`, and every prior method is `builds_on` or `compared_against`.
...
3. Sweep the five clusters in order: the_method (the root — `contribution`, `contribution_resource`, or `contribution_finding` — then each `component`), prior_art (`builds_on`, then `compared_against`), testbed (`dataset` / `benchmark` / `task`, plus `theoretical_setting` / `structural_class` for theory papers), yardsticks (`metric`), the_problem (the single `problem` node). If the paper reports results, the testbed and yardstick clusters must not be empty — find what the metrics were measured on.
