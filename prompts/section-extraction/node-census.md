...
**Root:** the paper's primary deliverable is a root node — tagged `contribution` (a method/system) or `contribution_resource` (a dataset/benchmark deliverable). Almost always there is **exactly one**: if several methods could qualify, only the overall primary one is the root; every part is a `component`, and every prior method is `builds_on` or `compared_against`.
...
5. Tag the root node(s) — `role: contribution` for a method/system deliverable, or `role: contribution_resource` for a dataset/benchmark deliverable. Almost always exactly one; mark two co-equal roots only for a genuine joint deliverable (see "Co-equal contributions").
