...
> **Provenance.** Production relation-pass prompt (section-ir-0.17), SC idiom. The pre-SC original is
> archived at `archive/legacy-prompts/section-ir-0.17-original/section-extraction/relation-pass.md`
> (intended semantic equivalent — same four edge types, matrices, carve-outs). `load_prompt()` sends
> the first fenced block (System) and the last fenced block (User); provenance markers are bare `§N`.
