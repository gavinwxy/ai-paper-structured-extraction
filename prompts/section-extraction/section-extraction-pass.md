...
> **Provenance.** Production Stage-C shared core (section-ir-0.17), SC idiom. The pre-SC original is
> archived at `archive/legacy-prompts/section-ir-0.17-original/section-extraction/section-extraction-pass.md`.
> `load_prompt()` sends the **first** fenced block (the shared System Prompt); the **last** fenced
> block (the User Prompt mirror) exists only to satisfy the loader — at runtime the content user prompt
> is built by `render_content_user_prompt`, which discards this file's user block. Provenance markers
> are bare `§N`.
...
`section_focus` tells you which of these your section does. You may also introduce a node the census missed — but **only of a unit type your `section_focus` owns** — giving it a fresh, correctly-prefixed id and extracting it as a full unit; **never mint a unit type another section owns** to patch a perceived gap. **`anchor_id` must name a unit you define in this section, and it must not be a Document.**
