...
- `ExperimentSetup` (array `experiment_setups`): the merged "what it ran on / under what configuration" type. Two flavours, told apart by `role`:
  - **substrate** (`role: dataset | benchmark | task`) — the datasets, benchmarks, and tasks the method is evaluated on. Materialize **every** substrate census node in `node_registry`, including `task` nodes, reusing each `node_id` and its `role`.
...
### Measure
Fields: `name`, `unit`, `setup_ids`, optional `comparison_direction`, and `scores`.
...
- `scores`: a flat array of `{variant, value, variance, system_id, setup_id}` — one entry per **system reported under this measure**, covering both the method family's own variants **and every baseline / prior-SOTA system the paper compares against**. One entry per system per split — do not pack multiple numbers into one entry.
...
- `role`: which experimental ingredient this is.
  - substrate — `dataset` (data trained/evaluated on), `benchmark` (a standardized dataset+protocol), `task` (the problem solved/evaluated). **Materialized** from census; reuse the node's `role`.
...
- For the headline contribution finding: emit `Finding --about--> mth:<contribution>`, pointing at the `node_registry` node whose role is `contribution`. (Downstream this is joined with the problem the contribution motivates to draw the closing problem→answer edge — you do not author that edge.)
