...
- `ExperimentSetup` (array `experiment_setups`): the merged "what it ran on / under what configuration" type — substrate roles are materialized from census, configuration roles are born here with a fresh `exp:` id. The role split and rules are in "### ExperimentSetup" below.
- `Finding` (array `findings`): the **headline contribution finding** (the paper's central established answer) plus the interpretive findings — ablation conclusions, sensitivity conclusions, limitations, failure modes. Born here — **except** a `contribution_finding` census root (see "headline contribution finding" below).
...
- A `contribution_resource` is the document root: the scores the paper reports on it are `scores[]` rows on Measures whose `setup_ids`/per-row `setup_id` point at this very ExperimentSetup, and the headline contribution finding points `about` it. For a `contribution_resource` root, the evaluation ON the resource IS the paper's own result: transcribe the headline evaluated-system rows (`setup_id` = the resource, `system_id` = the evaluated Method when censused) — the systems the paper headlines, not every minor variant; when the rows grade the resource by object/category rather than by system, transcribe the headline column's rows with the printed row label as `variant`; tables of those systems on OTHER datasets are context. When the census tagged co-equal roots (a contribution Method linked to the `contribution_resource` by `co_contribution`), the normal contribution-row rule takes priority: transcribe the contribution Method's own rows, and apply this carve-out only to tables where no contribution-Method row exists.
...
- **limitations and failure modes** (`role: failure_mode`) — ALWAYS one Finding per author-stated limitation, wherever it appears (a Limitations/Discussion/Conclusion/appendix sentence: "could not deal with X", "relies on Y", "does not extend to Z"), even when no table or number backs it. A hedged extension proposal ("can be extended to", "we leave to future work") is a future-work Finding (`role: descriptive`), not `failure_mode` — `failure_mode` requires a stated inability or dependency ("could not", "relies on", "does not extend").
- **cost-of-method** (`role: descriptive`) — the paper's stated cost of running the method (dollars, tokens, GPU-hours, latency) and its acceptability verdict; put the numbers in `effect_size`. ALWAYS emit when stated.
- **statistical-significance defense** (`role: comparative`, or `ablation_finding` inside an ablation) — a t-test / p-value / confidence statement defending a main comparison ("significant at the 1% level"). ALWAYS emit when stated.
...
- For each ablation finding: emit `Finding --about--> mth:<component>`, pointing at the specific component node from `node_registry` (not the whole-system root) when the ablation isolates one part. The link **to the data** is the `finding_ids` mount on the ablation Measure.
...
- Do not retype baseline / prior-SOTA / competitor rows into `scores[]` for a table-backed measure. They stay in the source table; you point at it with `source_table_marker`. Two exceptions: the headline evaluated-system rows ON a `contribution_resource` root (they ARE the paper's own result), and prose mode (no tables — transcribe every reported comparison row).
