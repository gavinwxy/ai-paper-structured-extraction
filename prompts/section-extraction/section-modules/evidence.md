...
- `Contribution` (array `contributions`): **only** `dataset`/`benchmark`/`finding`-kind Contributions here (method Contributions live in the method section). Materialize each such `con:` census node (reuse `node_id` + `kind`); you may add one the census missed. It hosts its own score rows — a Measure's `setup_id`/per-row `setup_id` may point at this `con:` id. **Do not mint a fresh dataset/benchmark Contribution for anything the method is merely trained or evaluated *on*** — a standard evaluation dataset or benchmark (CIFAR-10/100, ImageNet, COCO, GLUE), or an environment, simulator, game, or scenario (a control task, a grid world, "the Breakout scenario"). That is an `ExperimentSetup` (the evaluation frame), already a census `exp:` node: materialize/reference that `exp:` id and attach its scores via `setup_id` — never mint a parallel `con:*_eval` Contribution. A `dataset`/`benchmark` Contribution is reserved for one the paper itself *releases* as a deliverable.
