...
**Per-table procedure** — for EACH marker in the *Source-table index*, decide in order: (1) no contribution row and no finding evidenced (dataset statistics, notation, pure prior-work context) → emit NO Measure for that table (a censused `mea:` node must still be materialized — attach it to the table that actually reports it); (2) the table isolates a component, sweeps a hyperparameter, or probes sensitivity → `table_role: "ablation"`, `scores: []`, finding(s) in `finding_ids`; (3) otherwise → `table_role: "main_result"`, one Measure per metric, transcribe the contribution rows and emit `headline_result`. ALWAYS write `table_role` explicitly whenever `source_table_marker` is set.
...
- `table_role`: `main_result` or `ablation` (defaults to `main_result` if omitted).
  ...
  - `ablation` — a study that isolates a component, sweeps a hyperparameter, or probes sensitivity ("without attention", "−BN", "K = 2,4,8,16"). This role leaves `scores[]` empty (`[]`) — the verbatim table carries the whole grid. Mount the ablation Finding(s) via `finding_ids`.
