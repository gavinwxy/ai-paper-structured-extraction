...
Only `Method` units (array `methods`). Typical: 1–5 units — the primary Method plus its major component methods.
...
- Do create Method units for baselines the authors reconstruct, modify, or extend in detail ("we build upon ResNet by…") — the census should already list them.
