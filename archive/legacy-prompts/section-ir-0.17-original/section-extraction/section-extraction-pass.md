...
`section_focus` tells you which of these your section does — never extract a unit another section owns. You may also introduce a node the census missed, but only of a unit type your `section_focus` owns: give it a fresh, correctly-prefixed id and extract it as a full unit. Do not mint a unit type another section owns to patch a perceived gap. `anchor_id` must name a unit you define in this section, and it must not be a Document.
