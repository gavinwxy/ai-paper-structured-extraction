...
1. **Node census (stage A)**: `prompts/section-extraction/node-census.md`
   ...
   - Each node carries a single granular `role` drawn from four search clusters (the guiding principle "trace the method's life"): **the_method** (`contribution`/`contribution_resource`/`contribution_finding`, `component`), **prior_art** (`builds_on`, `compared_against`), **testbed** (`dataset`, `benchmark`, `task`), **yardsticks** (`metric`). The node's coarse `type` (`Method`/`ExperimentSetup`/`Measure`/`Finding`) and document root are **derived from `role`** — the census commits to one axis, and the `role` is then carried onto the materialized unit as its fine-grained differentia (0.9: `type` = generic scope, `role` = sub-axis). The `node_id` prefix (`mth:`/`exp:`/`mea:`/`fnd:`) follows from the role's type and is reused verbatim as the final unit id; each node also has a `salience` (`must`/`should`). The three root roles cover the three deliverable shapes — a **method** (`contribution`, `mth:`), a **resource** (`contribution_resource`, `exp:`; FG-1), or a **result/finding** (`contribution_finding`, `fnd:`; FG-5, an analysis paper with no proposed artifact).
...
- At least one census node is a root — `role: contribution` (a Method deliverable), `contribution_resource` (a dataset/benchmark deliverable), or `contribution_finding` (a result/finding deliverable — an analysis paper with no proposed artifact; FG-5, materialized as a Finding by the evidence section). Normally exactly one, but a paper with **co-equal primary contributions** (FG-7) tags each as a root and they are linked by `co_contribution` in stage B. `normalize_census_nodes` promotes the strongest node when none is tagged, and when several are tagged keeps every `must` root (demoting only weaker `should` mis-tags), so a missing contribution no longer hard-fails the census and a genuine co-contribution is no longer flattened to `component`.
- Assembly merges the stage-B relations with each content section's `relations[]` into the global list, then performs lossy-but-safe repairs logged to `extraction_notes.uncertain_assignments`: `_sanitize_unit_text` strips C0 control characters (incl. the NULL bytes some models emit for `·`/`×`) from unit string values; `_assign_roles_from_census` stamps the census-committed `role` onto each materialized Method/ExperimentSetup unit (born units author their own role); `_dedup_experiment_setups` merges same-name ExperimentSetups (rewriting relation endpoints); `_dedup_unit_ids` drops later duplicate definitions; `_drop_empty_sections` removes unit-less sections; `_normalize_provenance_markers` collapses fine-grained / ranged markers to the top-level `§N`/`§X` parent that contains them — dotted subsections (`§4.3`→`§4`, `§C.1`→`§C`), IEEE-style hyphenated subsections (`§IV-D`→`§IV`, `§III-F1`→`§III`), block ranges (`§107-108`/`§118-§120`→leading block `§107`/`§118`), glued appendix subsections (`§A4.2`→`§A`, `§G2`→`§G`), spelled-out appendices (`Appendix A.2`/`App. D.1`→`§A`/`§D`), and a stray `§` glued to a float word (`§Table 3`→`Table 3`, `§Figure 2 (a)`→`Figure 2 (a)`); a bare lettered appendix or Roman section (`§D`, `§IV`) is itself valid. `PROVENANCE_SOURCE_RE` accepts `§N`, `§X` (incl. a short lowercase section word like `§supp`), **and** genuine float references — `Table 5`/`Figure 8`/`Algorithm 1`/`Eq. 3`/`Listing 1`, the theorem-environment refs theory papers cite (`Lemma 2`/`Theorem 3.1`/`Corollary 1`/`Proposition`/`Definition`, FG-2), and a float carrying a panel suffix (`Figure 2 (a)`/`Figure 1 (R)`) — since a float names a real, citeable paper element (often the most precise source for a Measure's numbers), the renderer shows it verbatim, and nothing downstream resolves a marker back to the input, so it is no longer hard-failed; only free prose still fails (a `§` glued to a float word like `§Table 3` is normalized away first, above); `_repair_section_anchors` re-points non-local anchors; `_repair_score_refs` blanks dangling/wrong-type `scores[].system_id`/`setup_id` and prunes dangling/non-local/wrong-type entries from the Measure-level `setup_ids[]` list (dropped, not blanked — so a setup the model named in `setup_ids` but never materialized does not hard-fail an otherwise-sound measure); `_repair_unit_enums` normalizes degenerate / out-of-vocab values on the optional 0.10 enum fields — a blank `polarity:""` and a `Method.method_kind`/`Measure.objective_class`/score-row `value_kind` outside its controlled set are **dropped** (the field is simply absent when unsupported), while an invalid **required** `Finding.role` is coerced (a polarity word like `mixed` is moved to `polarity` and `role` floored to `descriptive`, else floored outright) — so a benign optional-enum quirk does not hard-fail an otherwise-sound extraction; `_dedup_relations`/`_drop_dangling_relations`/`_drop_invalid_relations` clean the global edge list against the relation matrix once the full unit set is known; `_drop_baseline_evaluates` drops `evaluates` edges pointing at a `compared_against` baseline (census-driven, so a measure's primary subject stays recoverable); `_assign_covers_entries` recomputes the census trace. Coverage is measured against census `must` nodes; an unmaterialized must-node surfaces in `extraction_notes.uncovered_items`.
...
- **FG-1 (non-Method contribution):** a benchmark/dataset deliverable is the census role `contribution_resource` (an `exp:` ExperimentSetup that *is* the document root and hosts its own score rows — no duplicate Method twin). `CONTRIBUTION_ROLES = {contribution, contribution_resource, contribution_finding}` (the third is FG-5, below). Non-data resource/taxonomy deliverables stay a Method with `method_kind ∈ {resource, taxonomy}`.
...
- **FG-9 (result-centric annotation):** optional `spine_summary.headline_result` names the paper's headline established result, lifted onto `Document.headline_result`.
- **FG-5 (Finding-as-root, structural):** the census may root an analysis/mechanistic paper on a `contribution_finding` node (a Finding, `fnd:` id) instead of inventing a hollow "Analysis" Method. The evidence section **materializes** it (reuses the `node_id` as the headline finding, authoring a real content `role`); `_assign_resolves` closes the arc by emitting that finding `--resolves-->` the Problem **directly** (no `about`→Method hop), and `_assign_roles_from_census` only stamps a census role that is valid for the unit type (so `contribution_finding` is not stamped onto the Finding). Method/resource/taxonomy deliverables still root on `contribution`/`contribution_resource` — validated 5/5 (incl. analysis-titled-but-real-method papers EPAGCL/SIFT/Diffusion-Veteran) stay Method-rooted; pure-analysis papers (e.g. "Is Value Learning the Bottleneck") root on the finding with the hollow Method eliminated.
...
`Method`, the substrate `ExperimentSetup` roles (`dataset`/`benchmark`/`task`), and `Measure` are census nodes (role-tagged in stage A, with `type` derived from `role`); `Problem`, `Finding`, and the configuration `ExperimentSetup` roles are born during content fill — **except** the one `contribution_finding` Finding root (FG-5), which is a census node the evidence section materializes by reusing its `node_id`.
