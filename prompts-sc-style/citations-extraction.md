# Citation-Relation Extraction (paper-level) — SC-style rewrite

Classify how THIS paper relates to each prior work it cites — a paper-level outward relation profile.

> **Style note.** Stylistic rewrite of `prompts/citations-extraction.md` in the
> `ie-pipeline-integrated-sc` idiom; semantics unchanged (same seven roles, same SKIP bucket, same
> tiebreaks, same gate). `load_prompt()` sends the first fenced block (System) and the last fenced
> block (User). The OUTPUT FORMAT CONTRACT is appended from the schema at runtime.
