...
- **Prose mode:** if the Source-table index contains no real `<table>` grids, omit `source_table_marker` and transcribe **all** explicitly reported comparison rows, **including baselines**.
...
3. **Materialize censused measures with no table support.**
   ...
   - if values appear only in prose/caption, transcribe the **stated** numbers in **prose mode** — **no `source_table_marker`, not `main_result`**;
...
- **prose mode**: include **all** explicitly reported comparison rows.
...
- Use **precise names**, not vague descriptions; add a `description` only for custom or non-obvious setups.
