...
>
> **【后续修复 · Pass 1,2026-06-01,committed `54279a2`】co_contribution 已修复。** 根因是 census 单根不变式(非边),遂放宽:`validate_census` `root_count != 1`→`< 1`(≥1,>1 合法);`normalize_census_nodes` 保留每个 **`must`** 根、仅降级较弱 `should` 误标(防根泛滥);`node-census.md` 加 co-equal 豁免("罕见;须真正共生可分;存疑取单根");`relation-pass.md` 兄弟变体→`compares_to`/`co_contribution` 而非 `part_of`。验证:013/035/074 全触发 `co_contribution`、假 `part_of` 消失、arc 跨共生贡献扇出、0 校验问题;过度触发 **0/6** 干净单贡献对照长第二根;129 单测绿(3 个旧单根测试改写);CLAUDE.md 同步;无 schema regen。
>
> **【后续修复 · Pass 2,2026-06-02,uncommitted】hollow-Method 已修复(Finding-as-root,加性、仍 0.10)。** 即 A1 census 重写,但**比预想轻**——evidence 本就发 `findings[]`,故 census Finding 根只是复用 `node_id` 物化。改动:新 census 角色 `contribution_finding`→type Finding(`fnd:`、the_method、入 `CONTRIBUTION_ROLES`);`NODE_TYPES`/`NODE_ID_PREFIX_BY_TYPE`/`SECTION_MATERIALIZED_NODE_TYPES[evidence]` += Finding;两处装配护栏:`_assign_roles_from_census` 仅在 census 角色对该 unit type 合法时才 stamp(否则 `contribution_finding` 会污染 evidence 写的内容角色),`_assign_resolves` 新增分支——根本身是 Finding 时直接 `resolves` Problem(about-join 不会触发)。提示词:node-census.md 第 3 条根路径 + findings-非节点豁免;evidence.md 按 id 物化;problem.md analysis 论文 `motivates` 指向被研究的方法。census schema regen。验证:**纯分析论文(019/059/078)根为 `contribution_finding`,空洞 Method 消失,arc 直接闭合**(019:`fnd:…bottlenecks --resolves--> prb:…`,内容角色 `comparative` 存活);**回归零误判:5/5 方法论文(含 analysis-titled-but-real-method 的 EPAGCL/SIFT/Diffusion-Veteran)仍 Method 根**;framework/taxonomy 交付(048/076)正确留 Method。9/10 valid;唯一 fail(037)是正交的既存 provenance-marker 问题("Table 1" 当 `§N`),非 Pass-2 回归。136 单测绿(+7 FindingRootTests)。**FG-5 残留清零。**
